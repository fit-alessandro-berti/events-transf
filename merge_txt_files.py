...
def merge_txt_files(input_dir: str, output_file: str, recursive: bool = True, encoding: str = "utf-8"):
    ...
    # Open the output file once (more efficient and avoids partial writes on crash)
    with open(output_path, "w", encoding=encoding) as outfile:
        # Decide whether to search recursively
        ...
        for txt_file in sorted(input_path.glob(pattern)):
            ...
            # Use relative path so the output is nicer and portable
            ...
            try:
                with open(txt_file, "r", encoding=encoding) as infile:
                    # Copy content line by line (preserves original line endings)
                    for line in infile:
                        outfile.write(line)
                # Optional: add a blank line between files for readability
                outfile.write("\n\n")
